...
class KoInterpreter:
    ...
    def visit_FuncDecl(self, node: FuncDecl) -> None:
        # Register top-level functions in the global lookup table.
        # Nested declarations remain reachable through lexical scope lookup.
    ...
    def visit_Call(self, node: Call):
        ...
        try:
            if func is None and node.name in self.functions:
...
class IRBuilder:
    ...
    def _build_function(self, func):
        ...
        self.module.functions[qualified_name] = func_ir
    ...
    def _build_expression(self, expr):
        ...
        if isinstance(expr, Literal):
            ...
            index_temp = self._build_expression(expr.index[0])
            result_temp = self.new_temp()
            ...
            self.emit(IROpcode.BUILD_TUPLE, len(expr.elements), None, result_temp, IRType.TUPLE, 0)
            return result_temp
            ...
            self.emit(IROpcode.BUILD_MAP, len(expr.mapping), None, result_temp, IRType.DICT, 0)
            return result_temp
        return None
